...
<section style="animation:ocfade .4s ease both;padding-top:56px">
    ...
    <div style="display:flex;flex-direction:column;gap:10px">
        @forelse ($this->fichas as $f)
            ...
            >
                <span style="font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:16px;color:#4a5567;flex:none;width:26px">{{ str_pad($f->number, 2, '0', STR_PAD_LEFT) }}</span>
                <span style="flex:1;min-width:0">
                    <span style="display:block;font-size:16px;font-weight:600;color:#fff">{{ $f->objection }}</span>
                    <span style="display:block;font-size:12.5px;color:#8b95a3;margin-top:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ $f->meaning }}</span>
                </span>
                <span style="{{ $this->chipStyle($f->type) }}"><span style="{{ $this->dotStyle($f->type) }}"></span>{{ $f->type->short() }}</span>
    ...
    @teleport('body')
      <div>
        ...
        <div style="position:fixed;top:0;right:0;height:100vh;width:680px;max-width:94vw;background:#0f1319;border-left:1px solid rgba(255,255,255,.1);z-index:51;overflow-y:auto;animation:ocdrawer .28s cubic-bezier(.2,.8,.2,1) both;box-shadow:-30px 0 80px rgba(0,0,0,.5)">
            <div style="position:sticky;top:0;background:{{ $this->ficha->type->headGradient() }};border-bottom:1px solid rgba(255,255,255,.08);padding:24px 30px;z-index:2">
                <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px">
                    <div>
                        <div style="font:600 10px 'IBM Plex Mono',monospace;letter-spacing:.16em;color:oklch(0.80 0.13 82)">FICHA {{ str_pad($this->ficha->number, 2, '0', STR_PAD_LEFT) }} · {{ strtoupper($this->ficha->categoria->label) }}</div>
                        <div style="font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:30px;letter-spacing:-.02em;color:#fff;margin-top:8px;line-height:1.05">“{{ $this->ficha->objection }}”</div>
                    </div>
                ...
                <div style="margin-top:14px"><span style="{{ $this->chipStyle($this->ficha->type) }}"><span style="{{ $this->dotStyle($this->ficha->type) }}"></span>{{ $this->ficha->type->label() }}</span></div>
            </div>
            ...
            <div style="padding:26px 30px 60px">
                ...
                <div style="font-size:15px;line-height:1.65;color:#c3ccd6;margin:9px 0 26px">{{ $this->ficha->meaning }}</div>

                @if (!empty($this->ficha->worst_case))
                    <div style="border-left:3px solid oklch(0.58 0.14 28);background:oklch(0.58 0.14 28 / .08);border-radius:0 9px 9px 0;padding:15px 18px;margin-bottom:14px">
                        ...
                        @foreach ($this->ficha->worst_case as $p)
                ...
                @if ($this->ficha->question)
                    <div style="background:oklch(0.72 0.14 78 / .1);border:1px solid oklch(0.72 0.14 78 / .22);border-radius:11px;padding:18px 20px;margin-bottom:24px">
                        ...
                        <div style="font-family:'Space Grotesk',sans-serif;font-weight:600;font-size:20px;color:#fff;margin-top:8px;line-height:1.3">“{{ $this->ficha->question }}”</div>
                    </div>
                @endif

                @if (!empty($this->ficha->branches))
                    ...
                    <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:26px">
                        @foreach ($this->ficha->branches as $r)
                ...
                @if ($this->ficha->closing)
                    <div style="background:#141a24;border:1px dashed rgba(255,255,255,.16);border-radius:11px;padding:18px 20px;margin-bottom:20px">
                        ...
                        <div style="font-size:15px;line-height:1.6;color:#e7ebf0">“{{ $this->ficha->closing }}”</div>
                    </div>
                @endif
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
                    ...
                    <div style="background:oklch(0.55 0.11 155 / .07);border-radius:10px;padding:15px 18px">
                        ...
                        <div style="font-size:13px;line-height:1.55;color:#a0b0a6">{{ $this->ficha->tip }}</div>
                    </div>
                </div>
            </div>
        </div>
